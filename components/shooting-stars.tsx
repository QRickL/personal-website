...
export function ShootingStars() {
  ...
  useEffect(() => {
    const createStarGroup = () => {
      ...
      const baseTop = Math.random() * 60 + 10 // 10-70% from top
      const newStars: Star[] = []
  ...
  return (
    <div className="pointer-events-none fixed inset-0 z-0">
      {stars.map((star) => (
        <div
          key={star.id}
          className="absolute -left-20 w-16 h-16 rotate-[30deg]"
          style={{
            top: star.top,
            animation: `shootingStar ${star.duration}s ease-out ${star.delay}s forwards`,
          }}
        >
          <Image src="/shooting-star.png" alt="" width={64} height={64} className="w-full h-full" />
        </div>
      ))}
    </div>
  )
